 ...
 * Создаёт структуру, числа, которые заполнены нулями. (Нужны для нормализации числа)
 * @param[in] _size Размер массив, являющийся членом созданной структуры. (digits) 
 * @return Нуль-Натурально число с выделенным массивом чисел, заполненные нулями. А также с размером параметра `_size`
 */
...
/**
 * @brief Выполняет операцию `побитового Строгое ИЛИ` между Нуль-Натуральным числом и простым числом.
 * @param[out] _inatural Нуль-Натуральное число, к которому применяется битовое Исключающие ИЛИ. ${Change}
 * @param[in]  _inum     Число, которое используется для `XOR`.
 * \n
 * Вы можете использовать отрицательные числа, но они могут быть идентичны себе же.
 * @return Ничего не возвращает. Результат операции является параметр `_inatural`
 */
...
/**
 * @brief Выполняет операцию `побитового Строгое ИЛИ` между двумя Нуль-Натуральными числами.
 * @param[in] _inatural_1 Первое Нуль-Натуральное число
 * @param[in] _inatural_2 Второе Нуль-Натуральное число
 * @return ${Dynamic | Nullable} Результат, аналогичный между числами (result = _inatural_1 ^ _inatural_2)
 */
